...
def game_loop(running=True):
    ...
    while running:
        if game.gameRounds.score == 1:
            rprint(
                    f"""[blue]{Game.playerStats.name}[/blue]"""
                    """ VS """
                    f"""[red]{Game.opponentStats.name}[/red]"""
                )
        if game.gameRounds.score >= 10:
            rprint(
                    f"""[blue underline]{game.playerStats.name}'s[/blue underline]"""
                    f""" Score: {game.playerStats.score}"""
                )
            rprint(
                    f"""[red underline]{game.opponentStats.name}'s[/red underline]"""
                    f""" Score: {game.opponentStats.score}"""
                )
            rprint(
                    f"""{game.gameDraws.score} [yellow underline]"""
                    f"""{game.gameDraws.name}(S)[/yellow underline]"""
                )
